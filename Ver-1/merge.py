...
while running:
    ...
    if random.randint(0, 10) == 0:
        ...
        p_items.append(p_it[p_itemIndex][(x,y)])  # 1300, random.randint(0, screen_height - p_it[itemIndex].rect().size[1])]
        if random.randint(0, 10) == 0:
            n_itemIndex = random.randint(0, 2)
        n_items.append(
            n_it[n_itemIndex].itemlist)  # 1300, random.randint(0, screen_height - n_it[itemIndex].rect().size[1])])

        # print("추가된 아이템 길이",len(p_items) ,"/n")
        for i in p_items:  # [item]
            while flag:
                i.setRandomXY_pos()  # 한번만 처리
                flag = False
        i.shiftLeft()  # 계속 나아가야함
        if i.x_pos <= 0:
            p_items.remove(i)
            flag = True
